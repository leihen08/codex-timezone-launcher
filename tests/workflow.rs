use chatgpt_timezone_launcher::discovery::ClientTarget;
...
fn running_client_saves_selection_but_never_launches_or_terminates_it() {
    ...
    let outcome = save_and_launch_with(
        ...
        || {
            Ok(ClientTarget::DesktopExecutable(PathBuf::from(
...
fn stopped_client_is_launched_with_validated_timezone() {
    ...
    let outcome = save_and_launch_with(
        ...
        |target, timezone| {
            *launched.borrow_mut() =
                Some((target.executable().to_path_buf(), timezone.to_string()));
            Ok(())
        },
    )
    .unwrap();
...
fn invalid_timezone_stops_before_any_side_effect() {
    ...
    let result = save_and_launch_with(
        ...
        || {
            side_effect.set(true);
            Ok(ClientTarget::DesktopExecutable(PathBuf::new()))
        },
        |_| Ok(false),
        |_: &ClientTarget, _| Ok(()),
    );
